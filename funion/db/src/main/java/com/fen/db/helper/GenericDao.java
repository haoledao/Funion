...
import com.fen.db.tool.PagingTool;
...
class GenericDao<T> implements IBaseDao<T> {
	...
	private SQLiteTool mDb;
	
	public GenericDao(SQLiteTool db, Class<?> modelClazz) {
		this.mDb = db;
	...
	@Override
	public PagingTool<T> pagingQuery(String[] columns, String selection,
		...
		PagingTool<ResultSet> queryList = mDb.pagingQuery(mTableName, columns, selection, selectionArgs,
		...
		PagingTool<T> resultList = new PagingTool<T>();
	...
	@Override
	public PagingTool<T> pagingQuery(String selection, String[] selectionArgs, int page, int pageSize) {
